...
function CountThrow()
{
	...
	if (user_lang=="nl")
		{
		if (wcount == 1) {document.getElementById("message").value = "Klik op een dobbelsteen om deze te behouden."; window.setTimeout('D6.setButtonLabel("2de worp")', 1000); }
		if (wcount == 2) {window.setTimeout('D6.setButtonLabel("Laatste worp")', 500);}
		if (wcount > 2) {document.getElementById("message").value = "Voer je score in!"; D6.setButtonLabel("none"); }
	}	
	else
	{
...
function NextRound()
{
 	if (round<39)
	{
		...
		if (user_lang=="nl")
		{
				D6.setButtonLabel("Volgende ronde");
			document.getElementById("message").value = "Volgende ronde. Werp de dobbelstenen!";
		}		
...
function UndoMove() {
	...
	if (user_lang=="nl")
	{
		document.getElementById("message").value = "Opgave geannuleerd . Gelieve je punten te plaatsen.";
	}	
	else
	if (user_lang=="de_x_sie")
	{
